...
# Auto-populate pitcher/batter teams from the active session based on inning half
_active_session = next(s for s in sessions if s["id"] == active_session_id)
if half == "top":
    _auto_pt = _active_session.get("home_team")
    _auto_bt = _active_session.get("away_team")
else:
    _auto_pt = _active_session.get("away_team")
    _auto_bt = _active_session.get("home_team")
_team_ctx = (half, active_session_id)
if st.session_state.get("_team_ctx") != _team_ctx:
    if _auto_pt in utils.TEAMS:
        st.session_state["pt"] = _auto_pt
    if _auto_bt in utils.TEAMS:
        st.session_state["bt"] = _auto_bt
    st.session_state["_team_ctx"] = _team_ctx
